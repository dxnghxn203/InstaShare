...
public class WidgetActivity extends AppCompatActivity {
    ...
    private void getListFriend() {
        DatabaseReference databaseRef = FirebaseDatabase.getInstance().getReference().child("friend");
        databaseRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    String key = String.valueOf(snapshot.getKey());
                    int state = Integer.parseInt(String.valueOf(snapshot.getValue()));
                    String[] userUid = key.split("_");
                    if (state == 1) {
                        if (userUid[0].equals(user.getUid()))
                            listUid.add(userUid[1]);
                        else if (userUid[1].equals(user.getUid()))
                            listUid.add(userUid[0]);
                    }
                }
